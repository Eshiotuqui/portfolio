import { useParams } from "react-router-dom";
import { projects } from "../../../constants/Projects";

interface Params {
  [key: string]: string | undefined;
  id: string;
}

const Media = () => {
  const { id } = useParams<Params>();
  const project = projects.find((p) => p.id === id);

  if (!project) return null;

  return (
    <section id="Media" className="w-full flex flex-col lg:gap-9 gap-6">
      <div className="flex flex-col relative">
        <div
          className={`"lg:max-h-[150svh] "
          } overflow-hidden flex flex-center lg:gap-9 gap-6 `}
        >
          <source src={`/media/${project.id}/mobile.png`} type="image/png" />
          <img
            src={`/media/${project.id}/mobile.png`}
            className="w-fit h-fit object-cover"
            alt="Landing page"
            loading="lazy"
          />
          <source src={`/media/${project.id}/mobile2.png`} type="image/png" />
          <img
            src={`/media/${project.id}/mobile2.png`}
            className="w-fit h-fit object-cover"
            alt="Landing page"
            loading="lazy"
          />
        </div>
      </div>

      <div className="flex flex-col lg:gap-9 gap-6">
        <div className="flex lg:flex-row flex-col lg:gap-9 gap-6">
          <div className="w-full h-fit  overflow-hidden lg:rounded-3xl rounded-2xl border-2 border-gray-50">
            <picture className="w-[20%] h-fit lg:rounded-3xl rounded-2xl object-cover border-2 border-gray-50 overflow-hidden">
              <source
                src={`/media/${project.id}/mobile.png`}
                type="image/png"
              />
              <img
                src={`/media/${project.id}/mobile.png`}
                className="w-full h-fit object-cover"
                loading="lazy"
                alt="Landing page"
              />
            </picture>
          </div>
          <div className="w-full  overflow-hidden lg:rounded-3xl rounded-2xl border-2 border-gray-50">
            <picture className="w-[20%] h-fit lg:rounded-3xl rounded-2xl object-cover border-2 border-gray-50 overflow-hidden">
              <source
                src={`/media/${project.id}/mobile.png`}
                type="image/png"
              />
              <img
                src={`/media/${project.id}/mobile.png`}
                className="w-full h-fit object-cover"
                loading="lazy"
                alt="Landing page"
              />
            </picture>
          </div>
        </div>
        <div className="flex lg:flex-row flex-col lg:gap-9 gap-6">
          <div className="w-full  overflow-hidden lg:rounded-3xl rounded-2xl border-2 border-gray-50">
            <source src={`/media/${project.id}/test3.png`} type="image/png" />
            <img
              src={`/media/${project.id}/test3.png`}
              className="w-full h-fit object-cover"
              alt="Landing page"
              loading="lazy"
            />
          </div>
          <div className="w-full overflow-hidden lg:rounded-3xl rounded-2xl border-2 border-gray-50">
            <source src={`/media/${project.id}/test4.png`} type="image/png" />
            <img
              src={`/media/${project.id}/test4.png`}
              className="w-full h-fit object-cover"
              alt="Landing page"
              loading="lazy"
            />
          </div>
        </div>
        <div className="w-full aspect-video overflow-hidden lg:rounded-3xl rounded-2xl border-2 border-gray-50">
          <source src={`/media/${project.id}/desk.png`} type="image/png" />
          <img
            src={`/media/${project.id}/desk.png`}
            className="w-full h-fit "
            alt="Landing page"
            loading="lazy"
          />
        </div>
      </div>
    </section>
  );
};

export default Media;
